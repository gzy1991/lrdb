...
define(['jquery','PDUtilDir/dialog','PDUtilDir/grid','PDUtilDir/util','LrdbAwardDir/awardSessionSupport'],function($,Dialog,Grid,Util,Session){
	...
	result.getNews = function($compile,$scope){
		var dialog = Dialog({
            ...
            buttons:[{name:'确定',callback:function(){
            			...
            			var bool = getNewsData($scope,selected);
            			if(bool){
            				dialog.hide();
            			}
	                }}
	...
	result.delNews = function($scope,index){
		...
		for(var i=0;i<$scope.session.newsArr.length;i++){
			$scope.session.newsArr[i].yearThNewsSortNumber = i;
		}
	};
	...
	var getNewsData = function($scope,selected){
		if(!Session.exist(selected,'newsId',$scope.session.newsArr)){
			for(var i=0;i<selected.length;i++){
				$scope.session.newsArr.push({newsId:'newsId',newsTitle:'newsTitle',newsAuthor:'newsAuthor',newsSource:'newsSource',newsTypeName:'newsTypeName'});
			}
			for(var j=0;j<$scope.session.newsArr.length;j++){
				$scope.session.newsArr[j].yearThNewsSortNumber = j;
			}
			$scope.$digest();
			return true;
		}else{
			Util.alert('重复选择已存在新闻');
			return false;
		}
	};
