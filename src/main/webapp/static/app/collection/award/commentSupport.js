...
define(['jquery','PDUtilDir/dialog','PDUtilDir/grid','PDUtilDir/util','LrdbAwardDir/awardSessionSupport'],function($,Dialog,Grid,Util,Session){
	...
	result.getComment = function($compile,$scope){
		var dialog = Dialog({
            ...
            buttons:[{name:'确定',callback:function(){
            			...
            			var bool = getCommentData($scope,selected);
            			if(bool){
            				dialog.hide();
            			}
	                }}
	...
	result.delComment = function($scope,index){
		...
		for(var i=0;i<$scope.session.comments.length;i++){
			$scope.session.comments[i].yearThCommentSortNumber = i;
		}
	};
	...
	var getCommentData = function($scope,selected){
		if(!Session.exist(selected,'commentId',$scope.session.comments)){
			for(var i=0;i<selected.length;i++){
				$scope.session.comments.push({commentId:'commentId',commentTitle:'commentTitle',commentOpusName:'commentOpusName',commentAuthor:'commentAuthor',commentDate:'2015-10-15'});
			}
			for(var j=0;j<$scope.session.comments.length;j++){
				$scope.session.comments[j].yearThCommentSortNumber = j;
			}
			$scope.$digest();
			return true;
		}else{
			Util.alert('重复选择已存在评论');
			return false;
		}
	};
