define(["jquery"],function($){
    ...
    return {
        directives : [
             ...
             * csDialog = {
             *   setting : {}, //dialog配置参数,参考dialog组件
             *   template : "url", //dialog内容模板地址
             *   afterLoad : function(){}, //模板加载完毕后处理函数
             *   onClose:function(){} //dialog关闭后的处理函数
             * },
             * ngModel 配置dialog内的变量模型,模板中使用时加前缀model
             */
            /*{
                name : 'csDialog',
                    ...
                    return {
                        ...
                        link: function(scope, element, attrs, ctrl) {
                            var config = scope.csDialog,
                                onClose = config.onClose?config.onClose:null,
                            ...
                            $(element).on("mouseup", function () {
                                require(["PDUtilDir/dialog","text!"+config.template],function(Dialog,template){
                                    //获取dialog对象
                                    var dialog = Dialog($.extend(config.setting,{
                                    	id : this.id
                                    }));
                                    ...
                                    dialog.show();
                                    //dialog关闭后处理函数
                                    dialog.$getDialog().on("hidden.bs.modal",function(){
             ...
             *      data : url,         //url数据接口地址
             *      onSelect : function(data,setElement){}  //该函数为需要对选中元素数据对象进行额外处理功能时(即用户自定义当前
             *                                              //列选中操作),data是当前数据对象;
             */
            {
                name : 'csTypeahead',
                    ...
                    return {
                        ...
                        link: function(scope, element, attrs, ctrl) {
                            ...
                            scope.$watch("ngModel", function(){
                                //config = config?config:scope.csTypeahead;
                                if(!!config&&!!config.data){
                                    lastContent = $(element).val();
                                    if(!!getModelStatus()){
                                        dataInit(scope.ngModel,config.onSelect);
                                    }
                                }
                                setModelStatus(true);
                            });
                            //实例化输入推荐
                            var unwatch = scope.$watch("csTypeahead",function(){
                                config = scope.csTypeahead;
                                ...
                                if(!!config.bindElementId){
                                    $("#"+config.bindElementId).on("click","."+eventClass,function(){
                            ...
                            //设置ngModel的值
                            function setModel(scope,value){
                                setModelStatus(false);
                                scope.ngModel = value;
                            }
                            //设置插件是否根据ngModel的改变进行赋值初始化(根据ngModel初始化显示值)
                            ...
                            function setElement(html,code){
                                ...
                                $("#"+config.bindElementId).append($html);
                            }
                            //ngModel变化时,控制element显示内容填充
                            function fillContent(data,callback){
                                if(!config.bindElementId){
                                    lastContent = data[0]?data[0][config.key.data]:lastContent;
                            ...
                            function dataInit(initData,callback){
                                ...
                                var obj={};
                                obj[(config.key.request?config.key.request:"ids")] = initData.join(",");
                                if(typeof config.data === "string"){
                                    $.ajax({
                                        url : config.data,
                                        type : "POST",
             ...
             */
            {
                ...
                func : function() {
                    return {
                        ...
                        link: function(scope, element, attrs, ctrl) {
                            var inputSelectObj = null,
                                id = element[0].id?element[0].id:"csInput"+Math.floor(Math.random()*1000000);
                            element[0].id=id;
                            //监听ngModel变化
                            scope.$watch("ngModel", function(){
                                if(!!inputSelectObj&&!!inputSelectObj.config){
                                    inputSelectObj.dataInit(dataTransfer(scope.ngModel));
                                }
                            });
                            scope.$watch("csInput.data", function(){
                                if(!!inputSelectObj&&!!inputSelectObj.config){
                                    inputSelectObj.refreshPanel(scope.csInput.data);
                                    inputSelectObj.dataInit(dataTransfer(scope.ngModel));
                                }
                            });
                            var unwatchConfig = scope.$watch("csInput", function(){
                                var config = scope.csInput;
                                if(!config){return false;}
                                require(["PDUtilDir/inputSelect"],function(inputSelect){
                                        inputSelectObj = inputSelect($.extend({
                                            type : "select",
                                            data : []
                                        },config,{
                                            id : id,
                ...
                func: ["$filter",function($filter){
                    ...
                    return {
                        ...
                        link: function (scope, element, attrs,ctrl) {
                            ...
                            //绑定日期选择组件
                            require(["PDModuleDir/bootstrap/plugins/datetimepicker/js/datetimepicker","css!DateCss"],function(){
                                $(element).focus(function(){
                                    //中文国际化
                                    if(!$.fn.datetimepicker.dates['cn']){
                                ...
                                //if($(element).next().length){
                                    var $icon = $('<span class="input-group-addon"><i class="fa fa-calendar"></i></span>').click(function(){
                                        $(element).datetimepicker('show');
                                    });
                                    $(element).after($icon);
                                //}
