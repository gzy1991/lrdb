...
define(['jquery','PDUtilDir/dialog','LrdbAwardDir/awardSessionSupport'],function($,Dialog,Session){
	...
	result.editScheduleGroup = function($compile,$scope,index){
		$scope.session.scheduleGroup = {groupName:'',order:$scope.session.scheduleGroups.length,schedules:[]};
		if(index !== undefined){
		...
		var dialog = Dialog({
            ...
            buttons:[{name:'确定',callback:function(){
            			...
            			$scope.session.scheduleGroups.sort(Session.compare('order'));
            			$scope.$digest();
            			dialog.hide();
	                }}
            ],
	...
	result.editSchedule = function($compile,$scope,pIndex,index){
		if(index !== undefined){
			...
			$scope.session.schedule = {order:$scope.session.scheduleGroups[pIndex].schedules.length};
		}
		var dialog = Dialog({
			id : 'editSchedule',
			cache : false,
            ...
            buttons:[{name:'确定',callback:function(){
            			$scope.session.schedule.date = $scope.session.schedule.startDate ? ($scope.session.schedule.startDate + ($scope.session.schedule.endDate ? ('至' + $scope.session.schedule.endDate) : '')) : '';
            			if(index !== undefined){
            			...
            			$scope.session.scheduleGroups[pIndex].schedules.sort(Session.compare('order'));
            			$scope.$digest();
            			dialog.hide();
	                }}
            ],
