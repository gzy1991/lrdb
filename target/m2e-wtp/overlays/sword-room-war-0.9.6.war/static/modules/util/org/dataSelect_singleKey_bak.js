 ...
 */
define(["PDUtilDir/dialog","text!PDUtilDir/org/template/dataSelect.html",
    ...
    var dataCacheObj = {},
        ...
        multiTypeSearch="/sword/org/orgSearchForOrgSelect",
        typeList = {
            user : {
                key : {
                    tree : {
    ...
    function getDataByAll(code,type,callback){
        $.ajax({
            type:"GET",
            url:getServer()+typeList[type].all+(code?"?code="+code:""),
    ...
    DataSelect.prototype.initTree = function($wrap,type){
        ...
        if(this.config.tagAsync){
            var async = this.config.tagAsync[type];
            if(async&&async.url){
                typeList[type].key.all = async.url;
                setting.async = {
                    dataType:"json",
    ...
    DataSelect.prototype.initDataList = function(dataArr,type){
        ...
        for(var i=0,cur,content;i<dataArr.length;i++){
            ...
            $li = $('<li id="data_'+(curData[key.id]||'')+'" title="'+content+'">'+
            ...
            if (this.selectData[type][curData[key.id]]) {
        ...
        if(this.config.dataRefill) {
            if(!initData){
                ...
                $.ajax({
                    ...
                    data:initData,
                    success:function(data){
                        if(!data){console.log("选中数据不存在");return}
    ...
    DataSelect.prototype.addSelectData = function(dataArr,type){
        ...
        for (var i = 0,curData,content; i < dataArr.length; i++) {
            curData = dataArr[i];
            content =(curData[key.name]||'无')+'/'+(curData[key.code]||'无')+'/'+(curData[key.pname]||'无');
            if(!!this.selectData[type][curData[key.id]]){continue}
            $li = $('<li id="select_'+(curData[key.id]||'')+'" title="'+content+'"><span class="text">'+content+'</span></li>');
    ...
    DataSelect.prototype.init = function(config){
        ...
        $.extend(this.config,{
            ...
            tagAsync : {
                user : {
                    param : ["deptTreeId"],
                    url : "/sword/org/dept/getAsyncDeptTree"
                }
            }
        },config);
