define(["jquery","PDUtilDir/util",
			...
			"css!ZTreeCss","css!PDUtilDir/css/localZTree"],function($,Util,Menu){
	/**
	 * init tree
	 */
	function init($tree,zSetting,zNodes){
		if(!zSetting){return false}
		//初始化setting
	...
	function destroy(treeId,callback){
		...
		if(!zTreeObj||!$treeWrapper.length){return false}
	...
	function renderTree($tree,zSetting){
        if(!($tree instanceof jQuery)){return false}
