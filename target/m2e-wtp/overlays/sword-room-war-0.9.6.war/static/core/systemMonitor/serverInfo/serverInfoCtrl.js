define([ "jquery","PDCoreDir/systemMonitor/serverInfo/jquery.form"], function($) {

	return function($compile, $scope) {
		require([ "PDUtilDir/util", "PDUtilDir/tool", "PDUtilDir/grid" ,"PDUtilDir/dialog"], function(Util,
				Tool, Grid,Dialog) {
...
//                   		componentsList.push({"swordInfoComponentsName":data.swordInfoComponents[i]});
          	 ...
          	 $("#uploadLicenseBtn").click(function(){
                 var form = $("form[name=uploadLicenseForm]");  
                 var options  = {
                     ...
                     success:function(data){
                    	 ...
                    	 if(resultObj.status==false){
                             ...
                             dialog.setBody(resultObj.message);
                             dialog.show();
                    			return;
                    	 }
 						legalLicense=false;
						$.ajax({
							...
							dataType:"json",
							data :{
								fileAbsolutePath: resultObj.fileAbsolutePath
							},
							success : function(data) {
								var componentsList="<ul>";
			                   	for(var i=0;i<data.swordInfoComponents.length;i++)
